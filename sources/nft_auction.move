module sui_sphere::nft_auction;


use sui::balance::{Self, Balance};
use sui::coin::{Self, Coin};
use sui::sui::SUI;
use sui_sphere::copyright_nft::CopyrightNFT; //Our module for creating NFTs.
...
public struct NFT_AUCTION has drop {}


fun init(otw: NFT_AUCTION, ctx: &mut TxContext) {
    let publisher = package::claim(otw, ctx);
    transfer::public_transfer(publisher, ctx.sender());

    let revenue_cap = RevenueCap {
        id: object::new(ctx),
        revenue_address: ctx.sender()
    };
    transfer::public_transfer(revenue_cap, ctx.sender());
}
